...
install.service.file() {
  ...
  echo 'ExecStart='${install_dir}'/artesky_srv --ip '${1} >> ${SRVC_FILE}
  echo 'Restart=always' >> ${SRVC_FILE}
  echo 'RestartSec=10' >> ${SRVC_FILE}
  echo '' >> ${SRVC_FILE}
  echo 'WorkingDirectory='${install_dir}' ' >> ${SRVC_FILE}
  echo '' >> ${SRVC_FILE}
...
mkdir -p ${install_dir}/artesky_bin
cd ${install_dir}/artesky_bin
git clone https://github.com/maudy2u/artesky_flat_box.git

printf "\n[-] Download and Install the header files\n\n"
mkdir -p ${install_dir}/artesky_bin/libartesky_SDK/1.0.0/include
cd ${install_dir}/artesky_bin/libartesky_SDK/1.0.0/include
...
rm -rf ./bin ./lib ./libartesky_SDK ./plugin ./artesky_flat_box ./artesky-flat
